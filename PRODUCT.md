...
## Stack
Go, GTK4 and the system WebKitGTK 6. Microsoft Teams uses an installed Chrome, Chromium, or Edge browser in app mode because Teams requires a supported Chromium WebRTC runtime.
...
Presets for WhatsApp, Microsoft Teams, Messenger, Instagram Direct, Telegram and other web applications, plus arbitrary HTTP(S) URLs. Multiple embedded accounts are independent. Teams opens the configured URL in the default Chrome profile as a navigation-free app window. Sidebar ordering persists and supports drag and drop. Ctrl+1 through Ctrl+9 follows sidebar positions. Disabling closes an embedded WebView while retaining profile data. Enabling opens it again. Browser data can be cleared per embedded instance.
