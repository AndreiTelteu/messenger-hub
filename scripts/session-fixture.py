...
<p><button onclick="notify()">Test desktop notification</button> <span id="notification-status"></span></p>
<script>
...
async function notify(){let status=document.getElementById('notification-status');let permission=await Notification.requestPermission();status.textContent='Permission: '+permission;if(permission==='granted')new Notification('Messenger Hub test',{body:'Desktop notifications are working.'});}
show();
</script></body></html>'''
